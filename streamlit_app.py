...
def parse_pdf_path(pdf_path: str, model_name: str) -> str:
    ...
    print(pdf_path)
    print(model_name)
    file = parse(pdf_path, parser_type="LLM_PARSE", raw=True, model=model_name)
...
def main():
    ...
    model_name = st.selectbox("Select Model", ["gemini-1.5-flash", "gpt"])  # Replace with actual model names

    if st.button('Process'):
        if url:
            with st.spinner('Processing URL...'):
                try:
                    parsed_content, file_name = parse_pdf_url(url, model_name)
            ...
            with st.spinner('Processing File...'):
                try:
                    ...
                    parsed_content, file_name = parse_pdf_path(temp_file_path, model_name)
